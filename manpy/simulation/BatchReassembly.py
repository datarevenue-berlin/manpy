...
class BatchReassembly(CoreObject):
    ...
    def initialize(self):
        ...
        self.Res = simpy.Resource(
            self.env, self.numberOfSubBatches
        ...
        while 1:
            while 1:
                ...
                receivedEvent = yield self.env.any_of(
                    [self.isRequested, self.interruptionStart, self.initialWIP]
                )
            ...
            if (
                len(self.getActiveObjectQueue()) == self.numberOfSubBatches
                and self.currentEntity.type != "Batch"
            ) or (
                self.isProcessingInitialWIP and self.currentEntity.type == "Batch"
    ...
    def reassemble(self):
        activeObject = self.getActiveObject()
        activeObjectQueue = (
            activeObject.getActiveObjectQueue()
    ...
    def canAccept(self, callerObject=None):
        ...
        if not callerObject:
            return activeObject.Up and len(activeObjectQueue) < self.numberOfSubBatches
        ...
        if len(activeObjectQueue) == 0:
            return activeObject.Up and (thecaller in activeObject.previous)
        else:
            return (
                activeObject.Up
                and len(activeObjectQueue) < self.numberOfSubBatches
                ...
                and activeObjectQueue[0].type != "Batch"
                and thecaller.getActiveObjectQueue()[0].batchId
                == activeObjectQueue[0].batchId
            )
    ...
    def haveToDispose(self, callerObject=None):
        ...
        if len(activeObject.next) == 1 or callerObject == None:
            return (
                len(activeObjectQueue) == 1 and activeObjectQueue[0].type == "Batch"
            )  # the control of the length of the queue is not needed

        thecaller = callerObject
        return (
            len(activeObjectQueue) == 1
