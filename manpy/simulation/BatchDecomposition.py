...
class BatchDecomposition(CoreObject):
    ...
    def run(self):
        while 1:
            ...
            while 1:
                ...
                self.expectedSignals["initialWIP"] = 1
                self.printTrace(
                    self.id, waitEvent="(isRequested), interruptionStart, initialWIP"
                )
            ...
            while len(self.getActiveObjectQueue()):
                if (
                    len(self.getActiveObjectQueue()) == 1
                    and self.next[0].isRequested.triggered
                ):
                    break
        ...
        for i in range(self.numberOfSubBatches):
            if i == self.numberOfSubBatches - 1:
                ...
                numberOfSubBatchUnits = (
                    batchNumberOfUnits // self.numberOfSubBatches + 1
                )
            alreadyAllocatedUnits += numberOfSubBatchUnits
            subBatch = SubBatch(
                str(activeEntity.id) + "_" + str(i),
                activeEntity.name + "_SB_" + str(i),
    ...
    def haveToDispose(self, callerObject=None):
        ...
        thecaller = callerObject
        return (
            len(activeObjectQueue) == self.numberOfSubBatches
