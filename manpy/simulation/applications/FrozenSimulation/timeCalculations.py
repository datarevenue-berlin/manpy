...
def availableTimeInterval_Manual(manualTime, tStart, availTime):
    ...
    if availTime[sortedTime[i]]["end"] - max(tStart, sortedTime[i]) <= dt.timedelta(
        seconds=0
    ):
        i += 1
...
def availableTimeInterval_MM(manualTime, autoTime, tStart, availTime):
    ...
    while (
        i < len(sortedTime)
        and (tCumulative + availTime[sortedTime[i]]["end"] - max(tStart, sortedTime[i]))
        < manualTime + autoTime
    ):
        ...
        if availTime[sortedTime[i]]["endMode"] == "EOS":
            if (
                i + 1 < len(sortedTime)
                and availTime[sortedTime[i + 1]]["preDay"] == sortedTime[i].date()
                and availTime[sortedTime[i + 1]]["startMode"] == "SOS"
            ):
                tCumulative += availTime[sortedTime[i]]["end"] - max(
                    tStart, sortedTime[i]
                )
            else:
                tCumulative = dt.timedelta(seconds=0)
    ...
    return (
        ...
        + autoTime
        - tCumulative
        + max(tStart, sortedTime[i])
        - max(tStart, startSorted),
    )
...
def availableTimeInterval_MA(manualTime, autoTime, tStart, availTime):
    ...
    while True:
        ...
        if autoTime:
            if (
                availTime[sortedTime[i]]["end"] - max(tManualEnd, sortedTime[i])
                <= autoTime
            ):
                if availTime[sortedTime[i]]["endMode"] == "EOS":
                    ...
                    if (
                        availTime[sortedTime[i + 1]]["startMode"] == "SOS"
                        and availTime[sortedTime[i + 1]]["preDay"]
                        == sortedTime[i].date()
                        and availTime[sortedTime[i + 1]]["end"]
                        - max(tManualEnd, sortedTime[i])
                        >= autoTime
                    ):
                        break
                    else:
...
def updateAvailTime(keyStart, reqTime, tStart, availTime):
    ...
    if tStart + reqTime < tempSave["end"]:
        availTime[tStart + reqTime] = {
            "end": tempSave["end"],
            "startMode": "IS",
            "endMode": tempSave["endMode"],
            "preDay": tempSave["preDay"],
        }
    ...
    # case of interval ending after previous end (i.e. automatic operations)...check if goes into following available interval
        ...
        if tStart + reqTime > sortedTime[i]:
            # repeat procedure
            availTime = updateAvailTime(
                sortedTime[i],
                reqTime - (sortedTime[i] - tStart),
                sortedTime[i],
                availTime,
            )
    ...
    if availTime[sortedTime[i]]["end"] >= tEnd:
        ...
        if availTime[sortedTime[i]]["endMode"] == "EOS":
            if i == len(sortedTime) - 1:
                ...
                availTime[sortedTime[i + 1]]["preDay"] == sortedTime[i].date()
                and sortedTime[i + 1] >= tEnd
            ):
                availTime = updateAvailTime(
                    sortedTime[i],
                    tEnd - max(tStart, sortedTime[i]),
                ...
                availTime[sortedTime[i + 1]]["startMode"] == "SOS"
                and availTime[sortedTime[i + 1]]["preDay"] == sortedTime[i].date()
                and availTime[sortedTime[i + 1]]["end"] >= tEnd
            ):
                availTime = updateAvailTime(
                    sortedTime[i],
                    tEnd - max(tStart, sortedTime[i]),
