    ...
    for proj in list(seqPrjDone.keys()):
        ...
        for part in list(seqPrjDone[proj].keys()):
            ...
            if seqPrjDone[proj][part] < len(Projects[proj][part]):
                ...
                if seqPrjDone[proj][part] == 0 or Projects[proj][part][
                    seqPrjDone[proj][part] - 1
                ]["id"] not in list(G.Schedule[G.simMode].keys()):
                    minStartTime = max(G.xlreftime, G.OrderDates[proj])
                else:
                    minStartTime = G.Schedule[G.simMode][
                        Projects[proj][part][seqPrjDone[proj][part] - 1]["id"]
                    ]["endDate"]
                ...
                if possibleOp:
                    ...
                    newOp["project"] = proj
                    newOp["part"] = part
                    if newOp["operation"] not in [
                        "INJM",
                        "MILL",
                        "EDM",
                        "INJM-MAN",
                    ]:  # newOp['personnel'].lower() != 'automatic':
