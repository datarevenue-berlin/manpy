...
def combSchedules(bothSchedList, combData):
    ...
    with open(
        "Results\\Combined Schedule.xlsx", "wb"
    ) as h:  # completion time, cycle time and delay info in json format
...
def importInput(jInput, excelInput, algorithmAttributes):
    ...
    maxODate = min(iter(list(OrderDates.items())), key=itemgetter(1))[1]
    ...
    for item in PMInfo:
        ...
        if pm in offShiftTimes:
            for unavailDate in list(offShiftTimes[pm].keys()):
                resAvailability[pm] = availableTime_Shift(
                    unavailDate,
                    offShiftTimes[pm][unavailDate]["endDate"],
                    resAvailability[pm],
                )
        if pm in takenPeriods:
            for unavailDate in list(takenPeriods[pm].keys()):
                resAvailability[pm] = availableTime_Shift(
                    unavailDate,
                    takenPeriods[pm][unavailDate]["endDate"],
                    resAvailability[pm],
                )
    ...
    for item in MachInfo:
        ...
        if mach in offShiftTimes:
            for unavailDate in list(offShiftTimes[mach].keys()):
                resAvailability[mach] = availableTime_Shift(
                    unavailDate,
                    offShiftTimes[mach][unavailDate]["endDate"],
        ...
        if mach in takenPeriods:
            for unavailDate in list(takenPeriods[mach].keys()):
                resAvailability[mach] = availableTime_Shift(
                    unavailDate,
                    takenPeriods[mach][unavailDate]["endDate"],
                    resAvailability[mach],
                )
...
def initGlobals():
    ...
    G.tabSchedule[G.simMode].headers = [
        "Project",
        "Part",
        "Task ID",
        "Station",
