...
class OrderDecomposition(CoreObject):
    ...
    def isInRouteOf(self, callerObject=None):
        ...
        if len(thecaller.getActiveObjectQueue()) > 0:
            ...
            if activeObject.id in activeEntity.remainingRoute[0].get(
                "stationIdsList", []
            ):
                return True
        return False
    ...
    def createOrderComponent(self, component):
        ...
        try:
            ...
            if (
                component.get("_class", "not found") == "manpy.Mould"
                or component.get("_class", "not found") == "manpy.OrderDesign"
            ):
                raise MouldComponentException(
                    "there is a mould/orderDesign in the componentList"
                )
            ...
            for element in route:
                ...
                for obj in G.ObjList:
                    for elementId in elementIds:
                        ...
                        if obj.id == elementId and (
                            obj.type == "Exit"
                            or type == "MouldAssemblyManaged"
                            or type == "MouldAssemblyBufferManaged"
                        ):
                            exitAssigned = True
            ...
            if not exitAssigned:
                if len(G.MouldAssemblyList) != 0:
                    ...
                    route.append(
                        {"stationIdsList": bufferIDlist}
                    )  # assign MouldAssemblyBuffers
                    route.append(
                        {"stationIdsList": assemblerIDlist}
