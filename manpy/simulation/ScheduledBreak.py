...
class ScheduledBreak(ObjectInterruption):
    ...
    def __init__(
        ...
        breakPattern=[],
        endUnfinished=False,
        receiveBeforeEndThreshold=0.0,
        rolling=False,
        lastNoBreakDuration=0,
        **kw
    ):
    ...
    def run(self):
        ...
        if float(self.remainingBreakPattern[0][0]) == self.env.now:
            ...
            self.victim.schedule.append(
                {"station": {"id": "on-break"}, "entranceTime": self.env.now}
            )
        ...
        while 1:
            if not self.victim.onBreak:
                timeToStartBreak = float(
                    self.remainingBreakPattern[0][0] - self.env.now
                )
                yield self.env.timeout(
                    timeToStartBreak - self.receiveBeforeEndThreshold
                ...
                if station:
                    ...
                    if (
                        not self.endUnfinished
                        and station.expectedSignals["processOperatorUnavailable"]
                    ):
                        self.sendSignal(
                    ...
                    if (
                        G.RouterList[0].__class__ is SkilledRouter
                        and station.id
                        in list(G.RouterList[0].expectedFinishSignalsDict.keys())
                        and not G.RouterList[0]
                        .expectedFinishSignalsDict[station.id]
                        .triggered
                    ):
                        self.sendSignal(
                            receiver=G.RouterList[0],
                            signal=G.RouterList[0].expectedFinishSignalsDict[
                                station.id
                            ],
                            sender=station,
                        )
                ...
                assert self.remainingBreakPattern[0][1] > self.env.now, (
                    ...
                    % (self.env.now, self.breakPattern)
                )
                yield self.env.timeout(
                    float(self.remainingBreakPattern[0][1] - self.env.now)
                )  # wait for the onShift
                self.victim.onBreak = False
                self.victim.totalBreakTime += (
