...
class ShiftScheduler(ObjectInterruption):
    ...
    def __init__(
        ...
        thresholdTimeIsOnShift=True,
        rolling=False,
        lastOffShiftDuration=10,
        **kw
    ):
    ...
    def run(self):
        ...
        if float(self.remainingShiftPattern[0][0]) != self.env.now:
            ...
            if issubclass(self.victim.__class__, CoreObject):
                ...
                self.victim.schedule.append(
                    {"station": {"id": "off-shift"}, "entranceTime": self.env.now}
                )
        ...
        while 1:
            if not self.victim.onShift:
                assert self.remainingShiftPattern[0][0] > self.env.now, (
                    "Incorrect shift defined for %s (%s)"
                    % (self.env.now, self.shiftPattern)
                )
                yield self.env.timeout(
                    float(self.remainingShiftPattern[0][0] - self.env.now)
                ...
                self.victim.totalOffShiftTime += (
                    self.env.now - self.victim.timeLastShiftEnded
                )
                ...
                yield self.env.timeout(
                    timeToEndShift - self.receiveBeforeEndThreshold
                ...
                if self.thresholdTimeIsOnShift:
                    ...
                    yield self.env.timeout(
                        self.receiveBeforeEndThreshold
                    ...
                    if self.endUnfinished and self.victim.isProcessing:
                        ...
                        if self.victim.endedLastProcessing in receivedEvent:
                            (
                                transmitter,
                                eventTime,
                            ) = self.victim.endedLastProcessing.value
